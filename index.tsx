...
const GENERAL_KPIS = {
  ...
  // Accepted the offer (内定承諾) but ultimately didn't join — a distinct, more costly outcome
  // than declining/withdrawing before ever accepting.
  acceptanceWithdrawn: { label: '内定承諾後辞退数', target: 0 },
};
...
// お見送り/選考辞退 (concluded, unsuccessfully). Used by computeStageAdvanceUpdate to decide
...
const STAGE_ADVANCE_KPI_KEYS: Partial<Record<PipelineStage, (keyof typeof GENERAL_KPIS)[]>> = {
  ...
  'カジュアル面談': ['documentScreeningPassed'],
  '1次面接': ['documentScreeningPassed'],
  '2次面接': ['firstInterviewPassed'],
  '最終面接': ['secondInterviewPassed'],
  '内定': ['finalInterviewPassed', 'offersExtended'],
  '内定承諾': ['placements'],
  'お見送り': ['declined'],
  '選考辞退': ['withdrawn'],
  '内定承諾後辞退': ['acceptanceWithdrawn'],
};
...
function getStageAdvanceKpiKeys(prevStage: PipelineStage, nextStage: PipelineStage): (keyof typeof GENERAL_KPIS)[] {
  ...
  if (EXIT_PIPELINE_STAGES.includes(nextStage)) return STAGE_ADVANCE_KPI_KEYS[nextStage] || [];
  ...
  const keys: (keyof typeof GENERAL_KPIS)[] = [];
  for (let i = prevIdx + 1; i <= nextIdx; i++) {
    const stageKeys = STAGE_ADVANCE_KPI_KEYS[FORWARD_PIPELINE_STAGES[i]];
    if (stageKeys) keys.push(...stageKeys);
  }
  return keys;
}
...
interface CompanyApplication {
  ...
  stage: PipelineStage;
  nextAction: string;
  // Manually-set date of the next scheduled interview/action for the CURRENT `stage` only,
  // shown on the pipeline calendar. Automatically cleared by computeStageAdvanceUpdate whenever
  // `stage` changes without this also being set to a new value in the same edit — it must never
  // silently carry over as if it were the new stage's date (see stageHistory for the historical
  // record of when past stages' interviews actually happened).
...
interface Candidate {
  ...
  ownerEmail?: string;
  ownerLabel?: string;
  // Whether 候補者推薦数 (candidatesSubmitted) has already been counted for this candidate —
  // a candidate-level flag (not per-application), set the first time ANY of their applications
  // advances from 打診 to 書類選考 (see STAGE_ADVANCE_KPI_KEYS). A candidate-level flag rather
  ...
  // Captured at the exact same moment exitRecorded flips true (see computeStageAdvanceUpdate) —
  // which of the three exit KPIs this candidate's single credited outcome was, and which
  // FORWARD_PIPELINE_STAGES phase (plus date) the deciding application had most recently been
  // at. Powers 歩留まり分析's「見送り・選考辞退の発生フェーズ」breakdown as ONE data point per
  // candidate rather than one per application — mirroring exitRecorded's own dedup and its
  // "only once no company is still in selection" timing, instead of a live per-application scan.
 ...
 * is credited — only genuine edits to an already-existing application's stage count, never a
 * stage an application/candidate was simply created at.
 */
function computeStageAdvanceUpdate(
  ...
  const updatedApplications = nextCandidate.applications.map(app => {
    ...
    // scheduledDate/scheduledTime hold the next scheduled interview/action for whichever stage
    // the application was AT — once the stage actually moves on, that date belonged to the
    // interview that just concluded, never to the new stage. Left in place, this used to make
    // the pipeline card/calendar show the just-finished interview's date as if it were the NEXT
    // one, for as long as the next date hadn't been decided yet. Only clear it when this save
  ...
  // (exitRecorded), the same pattern as recommendationRecorded, not per application. Unlike
  // Pass 1 above, this DOES count even a brand-new application/candidate — a loss is worth
  // recording even when it's the very first thing entered for this candidate. Only fires once
  // the candidate has no OTHER application still actively in progress elsewhere (see
  // ACTIVE_PIPELINE_STAGES); this is re-checked on every save, so once the last active
  // application resolves, any pending exits on other applications are reflected together at
  // that point. If the candidate ends up with a mix of exit types across different companies,
  // 内定承諾後辞退 (accepted, then didn't join — the most costly outcome) takes priority over
  // 選考辞退 (the candidate's own decision), which in turn takes priority over お見送り.
  //
  // Also captures exactly ONE (candidate.exitPhase, exitPhaseAt) pair at this same moment — the
  // phase the WINNING exit type's application most recently came from (see exitedFromStage in
  // Pass 1 above), picking whichever such application got furthest (highest
  // FORWARD_PIPELINE_STAGES index) if more than one company shares the winning exit type. This
  // powers 歩留まり分析's「見送り・選考辞退の発生フェーズ」breakdown as ONE data point per
  // candidate, credited at the exact same time and under the exact same dedup as the KPI actual
  // itself — not a separate, per-application, always-live count.
  let exitJustRecorded = false;
  let exitPhase: PipelineStage | undefined;
  let exitPhaseAt: string | undefined;
  let exitKpiKey: 'declined' | 'withdrawn' | 'acceptanceWithdrawn' | undefined;
  if (!nextCandidate.exitRecorded) {
    ...
    if (!hasActiveApplication) {
      ...
      if (exitedApps.length > 0) {
        exitKpiKey = exitedApps.some(a => a.stage === '内定承諾後辞退') ? 'acceptanceWithdrawn'
          : exitedApps.some(a => a.stage === '選考辞退') ? 'withdrawn'
          : 'declined';
        kpiDeltas[exitKpiKey] = (kpiDeltas[exitKpiKey] || 0) + 1;
        exitJustRecorded = true;

        const winningStage: PipelineStage = exitKpiKey === 'acceptanceWithdrawn' ? '内定承諾後辞退' : exitKpiKey === 'withdrawn' ? '選考辞退' : 'お見送り';
        const candidateApps = exitedApps.filter(a => a.stage === winningStage && a.exitedFromStage);
        const furthestApp = candidateApps.reduce<typeof candidateApps[number] | undefined>((best, app) => {
          if (!best) return app;
          return FORWARD_PIPELINE_STAGES.indexOf(app.exitedFromStage!) > FORWARD_PIPELINE_STAGES.indexOf(best.exitedFromStage!) ? app : best;
        }, undefined);
        if (furthestApp) {
          exitPhase = furthestApp.exitedFromStage;
          exitPhaseAt = furthestApp.exitedAt;
        }
      }
    }
  }
...
const SCOUT_ACHIEVEMENT_TIERS: { threshold: number; tier: ScoutAchievementTier; emoji: string; label: string }[] = [
  // 「120%達成」だと実際の達成率（tier.rate）が120%を大きく超えるとき「120%達成
  // （進行中312%）」のように矛盾して見えたため、「〜以上」を付けて閾値であることを明示
  // （ユーザーからの指摘で修正）。実数はrate側で別途示す。
  { threshold: 120, tier: 'great', emoji: '🏆🎉', label: '120%以上達成' },
...
// 「今週の達成まであと少しのメンバー TOP3」（ScoutProgressLeaderboard）専用の表示用達成率。
// 実際の達成判定（バッジ・お祝い演出・累計ログ）は上のcomputeGatedScoutRate/
// getGatedScoutAchievementTierによるボトルネック方式（対象媒体すべてが個別に基準を満たして
// 初めて達成）のまま変えない——ここを平均に変えると「1媒体だけ極端に低くても平均で達成扱いに
// なってしまう」事故につながるため。一方でTOP3表示は「まだ達成していない人のモチベーションを
// 上げる」ための演出なので、1媒体の出遅れがボトルネックとして全体の数字を過度に下げてしまう
// より、対象媒体の進捗の単純平均のほうが「あと少し感」が伝わりやすいというユーザー要望に対応。
const computeAverageScoutRate = (mediaRates: ScoutMediaRate[]): number | null => {
  const eligible = getEligibleScoutMediaRates(mediaRates);
  if (eligible.length === 0) return null;
  const total = eligible.reduce((sum, m) => sum + (m.actual / m.target) * 100, 0);
  ...
  return (
    <div className="monthly-jackpot-overlay" role="status" aria-live="polite" onClick={() => setVisible(false)}>
      ...
      <div className="monthly-jackpot-content">
        <img src={WEEKLY_ACHIEVEMENT_CELEBRATION_IMAGE_URL} alt="" className="monthly-jackpot-photo" />
...
const computeUserScoutAchievement = (
  ...
  return {
    ...
    // weeklyRate/monthlyRateはScoutProgressLeaderboard（TOP3）専用の表示値——上のweekly/
...
// 設定されている」ものをすべての過去期間にさかのぼって適用する——媒体別月次進捗の「目標は
...
const computeScoutAchievementPeriods = (
  ...
  if (entries.length === 0 || awardMediaIds.length === 0) return { weekly: [], monthly: [] };
  const earliest = new Date(Math.min(...entries.map(e => new Date(e.date).getTime())));
  const now = new Date();
  const gatedRateFor = (totals: KpiTotals, targets: Record<KpiKey, number>): number | null => computeGatedScoutRate(
    awardMediaIds.map(id => ({
      id,
      name: id,
      actual: totals[`${id}_scoutsSent` as KpiKey] || 0,
      target: targets[`${id}_scoutsSent` as KpiKey] || 0,
    }))
  );
  ...
  while (weekCursor.getTime() < currentWeekStart.getTime()) {
    ...
    weekEnd.setHours(23, 59, 59, 999);
    const rate = gatedRateFor(calculateTotalsForRange(entries, allMedia, weekCursor, weekEnd), weeklyTargets);
    if (rate !== null && rate >= 100) weekly.push(weekCursor.toLocaleDateString('sv-SE'));
...
const ScoutCumulativeLeaderboardPanel: React.FC<{ title: string; entries: ScoutCumulativeLeaderboardEntry[] }> = ({ title, entries }) => {
  if (entries.length === 0) return null;
  return (
    <div className="scout-cumulative-leaderboard">
      <h3 className="scout-cumulative-leaderboard-title">{title}</h3>
      <ol>
        {entries.map((e, i) => (
          <li key={e.email} className="scout-cumulative-leaderboard-item">
            <span className="scout-cumulative-leaderboard-rank">{i + 1}</span>
            <span className="scout-cumulative-leaderboard-name">
              {e.displayName}
...
const ScoutCumulativeLeaderboard: React.FC<{ weekly: ScoutCumulativeLeaderboardEntry[]; monthly: ScoutCumulativeLeaderboardEntry[] }> = ({ weekly, monthly }) => {
  if (weekly.length === 0 && monthly.length === 0) return null;
  return (
    <div className="scout-cumulative-leaderboard-group">
      <ScoutCumulativeLeaderboardPanel title="📈 週次達成 累計回数TOP3" entries={weekly} />
      <ScoutCumulativeLeaderboardPanel title="📈 月次達成 累計回数TOP3" entries={monthly} />
    </div>
  );
};
...
// （対象媒体の進捗の単純平均、computeAverageScoutRate）が高い順に並べ、上位3件を返す。
// 「まだ達成していないか」自体は表示用の平均値ではなく、実際の達成判定（achieved、
// getGatedScoutAchievementTierによるボトルネック方式）で判定する——平均だと1媒体だけ大きく
// 出遅れていても他媒体の超過分で100%を超えてしまい得るため、rate<100を条件にすると本当は
// 未達成の人がリストから漏れてしまう。週次・月次は独立に集計する——同じ人が週次では未達成・
...
const APP_CHANGELOG: ChangelogEntry[] = [
  ...
  },
  {
    date: '2026-08-29',
    items: [
      '選考予定日を「確定」「調整中」から明示的に選べるようにした。「確定」を選ぶとこれまで通りカレンダーで日時を入力でき、「調整中」を選ぶと「候補者ボール」「企業ボール」「その他」から選考日程の返信待ち状況を記録できる（「その他」を選ぶと自由記入欄が出ます）。選考情報の編集画面・候補者登録フォームの選考一覧・候補者パイプラインの候補者詳細（いずれも選考予定日の欄）すべてに反映され、選考状況バッジやそのツールチップにも「・候補者ボール」のように表示されるので、カードを展開しなくても今どちらの返信待ちか分かります。確定/調整中を切り替えると、使わなくなった側の入力内容（日時、またはボール選択・自由記入）は自動でクリアされます',
      '【不具合修正】お問い合わせ（バグ報告・改善要望）を新規投稿すると、その場では画面に表示されるもののGoogleドライブへの保存が完了する前に約2秒後の自動保存（他の編集と共通の保存処理）が投稿前の古い内容でファイル全体を上書きしてしまい、ページを再読み込みすると投稿が消えてしまうことがあった不具合を修正。投稿の保存処理を自動保存と同じ順番待ちの列に並べることで、互いに競合しないようにした',
    ],
  },
  {
    date: '2026-08-10',
  ...
  },
  {
    ...
    items: [
      ...
      '【不具合修正】候補者メモ（候補者単位のメモ一覧）で改行して入力しても、メモ欄を閉じる（編集を終える）と改行が失われ1行に詰まって表示される不具合を修正',
    ],
  },
  {
    date: '2026-07-27',
...
const HELP_CONTENT: Record<'member' | 'manager', { title: string; items: string[] }[]> = {
  member: [
    ...
    },
    {
      ...
      items: [
        ...
        '現職年収・希望年収・想定年収は万円単位で入力します（例: 650と入力→650万円と表示）。ただし想定年収は、選考企業から内定が出た後は提示額を正確に把握できるよう円単位（例: 6543210と入力→654万3,210円と表示）に切り替わります。',
  ...
  manager: [
    ...
    {
      title: 'BP/ミドル権限（チームメンバーの代理入力・代理編集）',
      items: [
        '「チーム管理」からメンバーに「BP/ミドル」権限を付与できます。',
        'BP/ミドルは、自分が所属するチームのメンバーの実績（KPI）を、実績カレンダーから代理で入力できます。',
        'BP/ミドルは、自分が所属するチームのメンバーの候補者パイプラインも代理編集できます（氏名・年収・メモ・選考企業の追加/編集/削除・非表示切り替え・掘り起しリスト登録など、本人と同等の操作が可能）。選考ステージが進んだ際のKPI実績は、代理編集したBP/ミドルではなく候補者本人（対象メンバー）の実績に加算されます。',
    ...
    },
    {
      ...
      items: [
        'BP/ミドルではなくても、チーム作成・編集権限を持つユーザーは、所属メンバーの候補者の「非表示（選考終了）」切り替えだけは代理で行えます（それ以外の項目の編集はBP/ミドルのみ）。',
      ],
    },
    {
      title: '媒体管理',
...
const TeamsModal: React.FC<{
    ...
    // Chat送信先）を折り畳めるようにする。既定は折り畳み（他のセクションと同じ既定）。
    const [collapsedTeamIds, setCollapsedTeamIds] = useState<Record<string, boolean>>({});
    const isTeamCollapsed = (teamId: string) => collapsedTeamIds[teamId] ?? true;
                ...
                <div className="modal-body">
                    ...
                    {isEditable && (
                        <div className="teams-permission-section">
                            ...
                            >
                                <span>BP/ミドル権限の管理</span>
                                <span className={`toggle-icon ${openSections.middle ? 'open' : ''}`}>▼</span>
                            </h4>
                            <div className={`collapsible-content ${openSections.middle ? 'open' : ''}`}>
                                <p className="modal-description">
                                    「BP/ミドル」に指定した人は、自分が所属するチームのメンバー全員の実績を代理入力できるようになります（個人実績タブの実績カレンダー上部から。入力した内容はそのメンバー本人のその日の実績として上書き保存されます）。
                                </p>
                                {userOptions.length === 0 ? (
                                    <p className="no-data-message">まだユーザーデータがありません。</p>
                                ) : (
                    ...
                    {teams.length === 0 ? (
                        ...
                        <ul className="user-management-list">
                            {teams.map(team => {
                                const teamCollapsed = isTeamCollapsed(team.id);
                                return (
                                <li key={team.id} className="user-management-item" style={{ flexDirection: 'column', alignItems: 'stretch' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', cursor: editingTeamId === team.id ? 'default' : 'pointer' }}
                                        onClick={() => { if (editingTeamId !== team.id) toggleTeamCollapsed(team.id); }}
                                    >
                                        {editingTeamId === team.id ? (
                                            ...
                                            <span className="user-management-name">
                                                {team.name}
                                                <small style={{ color: 'var(--text-muted-color)', marginLeft: '0.5rem' }}>{team.memberEmails.length}名</small>
                                            </span>
                                        )}
                                        <div className="user-management-actions" onClick={(e) => e.stopPropagation()}>
                                            {isEditable && (
                                                editingTeamId === team.id ? (
                                                    <>
                                            ...
                                            <button
                                                ...
                                                style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '0.3rem 0.5rem' }}
                                                aria-label={teamCollapsed ? `${team.name}を展開する` : `${team.name}を折りたたむ`}
                                                aria-expanded={!teamCollapsed}
                                            >
                                                <span className={`toggle-icon ${!teamCollapsed ? 'open' : ''}`}>▼</span>
                                            </button>
                                        </div>
                                    </div>
                                    <div className={`collapsible-content ${!teamCollapsed ? 'open' : ''}`} style={{ padding: teamCollapsed ? '0' : '0.75rem 0 0 0' }}>
...
const CandidateModal: React.FC<{
    ...
    // この場で選考ステージを「内定」以上に変更した場合も即座に反映されるよう、live の
    // candidate.applications から都度判定する（initialDataだけを見ない）。
    const offerReceived = hasReceivedOffer(candidate);
            ...
            <form id="candidate-form" className="modal-body" onSubmit={handleSubmit}>
              ...
              <div className="form-grid">
                ...
                <div className="form-group">
                    {offerReceived ? (
                        <>
                            <label htmlFor="expectedAnnualSalary">想定年収 (円)</label>
                            <input
                                type="number"
                                ...
                                step="1"
                                min={0}
                                value={candidate.expectedAnnualSalary ? Math.round(candidate.expectedAnnualSalary * 10000) : ''}
                                onChange={(e) => {
                                    const yen = e.target.value === '' ? 0 : Number(e.target.value);
                                    setCandidate(prev => ({ ...prev, expectedAnnualSalary: yen / 10000 }));
                                }}
                                placeholder="例: 6000000"
                            />
                            <p className="form-helper-text">内定が出ているため、実際の円単位で入力できます。</p>
                        </>
                    ) : (
                        <>
...
const GrossProfitSummary: React.FC<{
    ...
    // to exactly that selection, even if it includes an exit stage — that's an explicit choice.
    ...
    const grandTotal = useMemo(() => {
        const base = selectedStageFilters.length > 0
            ? visibleStageTotals
            : stageTotals.filter(s => !EXIT_PIPELINE_STAGES.includes(s.stage));
...
// 現職年収・希望年収、および内定前の想定年収は万円単位のまま表示する（1円単位の内訳までは
// 不要なため）。formatSalaryAsYenと違い、万円建ての値をそのまま「○○万円」として見せる。
// （想定粗利集計用のformatManYenとは別物 — こちらは未設定時に空文字を返す）
const formatSalaryAsManYen = (manYen: number | undefined): string => manYen ? `${manYen.toLocaleString()}万円` : '';
...
const PipelineCandidateCard: React.FC<{
  ...
  // 想定年収の入力単位切り替え判定 — 内定が出るまでは万円単位、出た後は円単位。
  const offerReceived = hasReceivedOffer(c);
        ...
        <div className="candidate-card-body">
            <div className="candidate-key-info">
                ...
                <div className="key-info-item">
                    <span>現年収</span>
                    {candidateIsOwn ? (
                        <InlineNumberField value={c.currentSalary} onCommit={(v) => commitCandidateField('currentSalary', v || 0)} placeholder="例: 500" ariaLabel="現職年収" step="any" unit="万円" />
                    ) : (
                        formatSalaryAsManYen(c.currentSalary) || 'N/A'
                    )}
                </div>
                <div className="key-info-item">
                    <span>年齢</span>
                ...
                <div className="key-info-item">
                    ...
                    {candidateIsOwn ? (
                        offerReceived ? (
                            ...
                            <InlineNumberField value={c.expectedAnnualSalary} onCommit={(v) => commitCandidateField('expectedAnnualSalary', v)} placeholder="例: 600" ariaLabel="想定年収" step="any" unit="万円" />
                        )
                    ) : (
                        (offerReceived ? formatSalaryAsYen(c.expectedAnnualSalary) : formatSalaryAsManYen(c.expectedAnnualSalary)) || 'N/A'
                    )}
                </div>
                <div className="key-info-item">
                    <span>確度</span>
              ...
              <div className="candidate-info-section">
                ...
                <div className="candidate-info-item">
                    ...
                    {candidateIsOwn ? (
                        ...
                        <span className="info-value">{formatSalaryAsManYen(c.currentSalary) || 'N/A'}</span>
                    )}
                </div>
                <div className="candidate-info-item">
                    <span className="info-label">希望年収</span>
                    {candidateIsOwn ? (
                        <InlineNumberField value={c.salary} onCommit={(v) => commitCandidateField('salary', v || 0)} placeholder="例: 650" ariaLabel="希望年収" step="any" unit="万円" />
                    ) : (
                        <span className="info-value">{formatSalaryAsManYen(c.salary) || '未設定'}</span>
                    )}
                </div>
                <div className="candidate-info-item">
                    ...
                    {candidateIsOwn ? (
                        ...
                        <span className="info-value">{(offerReceived ? formatSalaryAsYen(c.expectedAnnualSalary) : formatSalaryAsManYen(c.expectedAnnualSalary)) || '未設定'}</span>
                    )}
                </div>
                <div className="candidate-info-item">
...
const FUNNEL_EXCLUDED_GENERAL_KPI_KEYS: (keyof typeof GENERAL_KPIS)[] = ['declined', 'withdrawn', 'acceptanceWithdrawn'];
...
const FunnelAnalysisSection: React.FC<{
  ...
  // candidate's exitKpiKey/exitPhase/exitPhaseAt (see computeStageAdvanceUpdate's Pass 2), NOT
  ...
  const exitBreakdownByStage = useMemo(() => {
    ...
    scopedUsers.forEach(email => {
      ...
      (data.candidates || []).forEach(candidate => {
        if (!candidate.exitRecorded || !candidate.exitKpiKey || !candidate.exitPhase || !candidate.exitPhaseAt) return;
        if (!isDateInFunnelPeriod(candidate.exitPhaseAt, periodOverride)) return;
        const entry = breakdown.get(candidate.exitPhase) || { declined: 0, withdrawn: 0, acceptanceWithdrawn: 0 };
        entry[candidate.exitKpiKey] += 1;
        breakdown.set(candidate.exitPhase, entry);
      });
    });
    return breakdown;
    ...
    try {
      ...
      if (selectedMediaId === 'all') {
        lines.push(
          ...
          `${GENERAL_KPIS.acceptanceWithdrawn.label}: ${acceptanceWithdrawnTotal}件${recommendedTotal > 0 ? `（推薦数対比: ${(acceptanceWithdrawnTotal / recommendedTotal * 100).toFixed(1)}%）` : ''}`
        );
        const exitStageLines = FORWARD_PIPELINE_STAGES
          .map(stage => ({ stage, entry: exitBreakdownByStage.get(stage) }))
          .filter(({ entry }) => entry && (entry.declined > 0 || entry.withdrawn > 0 || entry.acceptanceWithdrawn > 0))
          .map(({ stage, entry }) => `${stage}: 見送り${entry!.declined}件・辞退${entry!.withdrawn}件・承諾後辞退${entry!.acceptanceWithdrawn}件`);
        if (exitStageLines.length > 0) {
          lines.push(`見送り・選考辞退・内定承諾後辞退の発生フェーズ: ${exitStageLines.join(' / ')}`);
        }
      }
        ...
        {selectedMediaId === 'all' && (
          <>
            ...
            <div className="all-users-table-container">
              <table className="all-users-table">
                ...
                <tbody>
                  ...
                  <tr>
                    <td>{GENERAL_KPIS.acceptanceWithdrawn.label}</td>
                    <td>{acceptanceWithdrawnTotal}</td>
                    <td>{recommendedTotal > 0 ? `${(acceptanceWithdrawnTotal / recommendedTotal * 100).toFixed(1)}%` : '—'}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="all-users-table-container">
              <table className="all-users-table">
                <thead>
                  <tr><th>直前のフェーズ</th><th>{GENERAL_KPIS.declined.label}</th><th>{GENERAL_KPIS.withdrawn.label}</th><th>{GENERAL_KPIS.acceptanceWithdrawn.label}</th><th>合計</th></tr>
                </thead>
                <tbody>
                  {FORWARD_PIPELINE_STAGES.map(stage => {
                    ...
                    const acceptanceWithdrawn = entry?.acceptanceWithdrawn || 0;
                    return (
                      <tr key={stage}>
                        <td>{stage}</td>
                        <td>{declined}</td>
                        <td>{withdrawn}</td>
                        <td>{acceptanceWithdrawn}</td>
                        <td>{declined + withdrawn + acceptanceWithdrawn}</td>
                      </tr>
                    );
                  })}
                </tbody>
...
const TeamChatReportPanel: React.FC<{
  ...
  const handleSend = async () => {
    if (!effectiveWebhookUrl || !pendingPeriod) return;
    setSendStatus('sending');
    setSendError(null);
    try {
      // 同じthreadKeyを使うことで、このチームのレポートは毎回同じスレッドへの返信として
      // 投稿される（スレッド自体はチーム管理の「スレッドを作成」で立てたもの）。期間の種類が
      // 変わっても同じスレッドにまとまる。
...
const App: React.FC = () => {
    ...
    try {
      ...
      teammates.forEach(({ email, data, driveFileId: teammateFileId }) => {
        merged[email] = {
          ...
          scoutAchievementLog: data.scoutAchievementLog,
        };
  ...
  // 前回のneedsAggregateDataを覚えておき、false→true（集計が必要な画面に新しく入った瞬間）
  // でだけ再取得する — 集計画面を開いたまま他の状態が変わって再レンダリングされるたびに
  // 取得し直す（＝hasFetchedAllUsersRefのような「一度だけ」ではなく、かつ無駄打ちもしない）
  // ための簡単なエッジ検知。
      ...
      if (effectiveNext) {
        for (const app of nextApps) {
          try {
            ...
            // お見送り/選考辞退/内定承諾後辞退はこの選考が終わったことを意味するので、
            // scheduledDateがまだ残っていてもタスクは残さない（削除して以後は再作成しない）。
            // そうしないと、見送りにした後もGoogleカレンダーのタスクに「保留中」のまま
            // 表示され続けてしまう。
            if (!app.scheduledDate || EXIT_PIPELINE_STAGES.includes(app.stage)) {
              if (existingTaskId) {
  ...
  const handleSaveCandidate = (candidateData: Candidate) => {
    ...
    setCurrentUserData(prevData => {
        ...
        const updatedCandidates = existing
            ? prevData.candidates.map(c => c.id === finalCandidate.id ? finalCandidate : c)
        ...
        return { ...prevData, candidates: updatedCandidates, entries: updatedEntries };
    });
    queueCandidateTasksSync(prevCandidate, finalCandidate);
  };
